...
export const BookingFlow = () => {
  ...
  useEffect(() => {
    ...
    if (areaIdentifier) {
      fetch(`${import.meta.env.VITE_API_URL}/api/passes/slots?areaId=${areaIdentifier}`)
        .then(res => res.json())
        .then(data => setReservedPassSlots(data.reservedSlots || []))
        .catch(err => console.error("Failed to fetch reserved pass slots:", err));

      fetch(`${import.meta.env.VITE_API_URL}/api/reservations/area/${areaIdentifier}`)
        .then(res => res.json())
    ...
    if (userId) {
      fetch(`${import.meta.env.VITE_API_URL}/api/reservations/user/${userId}`)
        .then(res => res.json())
        .then(data => {
          if (data.reservation) {
      ...
      fetch(`${import.meta.env.VITE_API_URL}/api/reservations/pending-penalty/${userId}`)
        .then(res => res.json())
        .then(data => {
          if (data.hasPending) {
  ...
  const handleReserveSlot = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        fetch(`${import.meta.env.VITE_API_URL}/api/reservations/calculate-eta?originLat=${latitude}&originLng=${longitude}&destLat=${destLat}&destLng=${destLng}`)
          .then(res => res.json())
          .then(data => {
            setEtaDetails(data);
            setIsProcessing(false);
  ...
  const confirmReservation = async () => {
    setIsProcessing(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/reservations/create`, {
        method: 'POST',
  ...
  const handlePayment = async () => {
    setIsProcessing(true);
    // Simulate payment gateway (Reduced delay)
