...
export const AdminPanel = () => {
  ...
  useEffect(() => {
    if (user?.role === 'admin') {
      ...
      fetch(`${import.meta.env.VITE_API_URL}/api/reservations/all-expired`)
        .then(res => res.json())
  ...
  const fetchPremiumPasses = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/passes/all`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleDeleteLot = async (id: string, name: string) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/parking-areas/${id}`, {
        method: "DELETE"
      });
      if (res.ok) {
